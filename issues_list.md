...
2. Space shortcut only toggles store state, doesn't start/stop audio App.jsx line 36: The Space key shortcut calls setIsPlaying(!isPlaying) which toggles the Zustand state, but it never calls audioEngine.playProgression() or audioEngine.stop(). The UI indicator changes but no audio plays/stops.
